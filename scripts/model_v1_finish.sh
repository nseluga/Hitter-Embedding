#!/bin/bash
# Run the rest of Phase D.5's compute unattended: wait out the rebuild sweep, then hand off to
# the step-5 scoring driver. Both halves already resume from their own state -- the sweep from
# results/model_v1/sweep_log.csv, the driver from the per-run diagnostics JSON -- so this can be
...
echo "=== model_v1_finish start $(date '+%F %H:%M') | JOBS=$JOBS THREADS=$THREADS ==="
...
awk -F, -v s="$STAGE" '$1==s && $4=="ok"' results/model_v1/sweep_log.csv | wc -l \
  | xargs echo "trained runs in ledger:"

STAGE="$STAGE" bash scripts/model_v1_step5_score.sh
echo "=== model_v1_finish done $(date '+%F %H:%M') ==="
