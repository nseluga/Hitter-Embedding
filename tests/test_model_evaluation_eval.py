...
from src.analysis import model_evaluation_eval, model_evaluation_price_draw, model_evaluation_structure
...
def test_weighted_ols_recovers_a_known_line():
    ...
    y = 0.3 + 1.7 * x
    intercept, slope, se = model_evaluation_eval.weighted_ols(x, y, np.ones_like(x))
...
def test_weighted_ols_weights_actually_bind():
    ...
    _, slope, _ = model_evaluation_eval.weighted_ols(x, y, heavy_first)
    assert slope == pytest.approx(2.0, abs=1e-3)
    _, flipped, _ = model_evaluation_eval.weighted_ols(x, y, heavy_first[::-1])
...
def test_weighted_ols_standard_error_is_positive_under_noise():
    ...
    _, slope, se = model_evaluation_eval.weighted_ols(x, y, np.ones_like(x))
    assert 0.0 < se < 0.1
...
def test_per_group_absorbing_rates_sum_to_one():
    pa_df = _fake_pa()
    rates = model_evaluation_eval.per_group_absorbing(pa_df, [2022, 2023])
...
def test_matched_population_reduces_to_the_shipped_number_on_the_full_set():
    ...
    frame = model_evaluation_eval.per_group_absorbing(pa_df, seasons)
...
def test_absorbing_rates_of_a_synthetic_chain_exhaust_the_pa():
    rates = query.absorbing_rates(model_evaluation_price_draw.to_aggregates(_fake_chain()))
...
def test_draw_gap_conserves_mass_in_every_state():
    ...
    shifted = model_evaluation_price_draw.apply_draw_gap(chain, gaps)
...
def test_more_ball_mass_means_more_walks_and_fewer_strikeouts():
    ...
    chain = _fake_chain()
    base = query.absorbing_rates(model_evaluation_price_draw.to_aggregates(chain))
    gaps = pd.DataFrame([{"balls": b, "strikes": s, "draw_gap_ball": 0.02,
                          "draw_gap_hbp": 0.0, "draw_gap_called_strike": -0.02}
                         for b, s in query.COUNT_STATES])
    after = query.absorbing_rates(model_evaluation_price_draw.to_aggregates(
        model_evaluation_price_draw.apply_draw_gap(chain, gaps)))
...
def test_draw_gap_is_antisymmetric_in_sign():
    ...
    gaps = pd.DataFrame([{"balls": b, "strikes": s, "draw_gap_ball": 0.01,
                          "draw_gap_hbp": 0.0, "draw_gap_called_strike": 0.0}
                         for b, s in query.COUNT_STATES])
    up = model_evaluation_price_draw.apply_draw_gap(chain, gaps, sign=1.0)
    down = model_evaluation_price_draw.apply_draw_gap(chain, gaps, sign=-1.0)
...
def test_channel_masks_partition_every_real_description():
    ...
    masks = model_evaluation_structure._channel_masks(descriptions, keep)
...
def test_channel_masks_reject_an_unclassified_description():
    ...
    with pytest.raises(AssertionError):
        model_evaluation_structure._channel_masks(descriptions, np.ones(2, dtype=bool))
...
def test_paired_bootstrap_of_identical_columns_is_exactly_zero():
    ...
    paired = model_evaluation_eval.paired_platoon_difference(frame, n_boot=200, seed=0)
...
def test_paired_interval_narrows_with_more_hitters():
    ...
    small = width(model_evaluation_eval.paired_platoon_difference(_platoon_panel(60, 2), n_boot=600, seed=0))
    large = width(model_evaluation_eval.paired_platoon_difference(_platoon_panel(240, 2), n_boot=600, seed=0))
