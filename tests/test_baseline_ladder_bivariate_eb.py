...
def synthetic_pa_table(n_hitters, mu, tau2, rho, sigma2, pa_range, seed=0,
    ...
    matrix = eb.sigma_matrix(np.asarray(tau2, dtype=float), rho)
...
def test_g1_recovers_parameters_at_real_sample_sizes():
    ...
    fitted = eb.fit(pa_df, eval_season=2024, rho_min_pa=1)["R"]
...
def test_g2_zero_signal_returns_zero_not_a_convergence_floor():
    ...
    n_star_split, _ = eb.implied_split_constant(fitted)
...
def test_g3_pooling_sigma2_moves_predictions_materially():
    ...
    matrix = eb.sigma_matrix(tau2, 0.85)

    per_cell = eb.posterior_mean(x, n_pa, mu, matrix, np.array([0.2374, 0.2610]))
    pooled = eb.posterior_mean(x, n_pa, mu, matrix, np.array([0.2610, 0.2610]))
...
def test_g4_rho_zero_reduces_to_independent_shrinkage():
    ...
    shrunk = eb.posterior_mean(x, n_pa, mu, eb.sigma_matrix(tau2, 0.0), sigma2)
    n_star = sigma2 / tau2
...
def test_g5_zero_pa_side_matches_the_analytic_limit():
    ...
    mu, tau2, sigma2 = np.array(REAL_MU), np.array(REAL_TAU2), np.array(REAL_SIGMA2)
    rho = 0.85
    matrix = eb.sigma_matrix(tau2, rho)
    x_r, n_r = 0.420, 600.0

    shrunk = eb.posterior_mean(np.array([[np.nan, x_r]]), np.array([[0.0, n_r]]), mu, matrix, sigma2)
...
def test_g5_no_data_at_all_returns_the_prior_mean():
    mu, tau2, sigma2 = np.array(REAL_MU), np.array(REAL_TAU2), np.array(REAL_SIGMA2)
    shrunk = eb.posterior_mean(np.array([[np.nan, np.nan]]), np.array([[0.0, 0.0]]),
                               mu, eb.sigma_matrix(tau2, 0.85), sigma2)
...
def test_g5_shrinkage_is_monotone_in_exposure():
    ...
    shrunk = eb.posterior_mean(x, n_pa, mu, matrix, sigma2)[:, 0]
...
def test_g6_impossible_correlation_is_rejected():
    with pytest.raises(AssertionError, match="positive semidefinite"):
        eb.sigma_matrix(np.array(REAL_TAU2), 1.5)
...
def test_g6_rho_pinned_at_the_ceiling_is_flagged_and_derives_nothing():
    ...
    record = eb._assemble("R", {"L": 0.30, "R": 0.32}, impossible, {"L": 0.24, "R": 0.26},
    ...
    healthy = eb._assemble("R", {"L": 0.30, "R": 0.32}, impossible, {"L": 0.24, "R": 0.26},
                           covariance=0.0013 * 0.85, n_hitters=500, n_rho_hitters=300)
    assert not healthy["rho_at_bound"]
    assert np.isfinite(eb.implied_split_constant(healthy)[0])
...
def test_g7_fit_ignores_the_evaluated_season_entirely():
    ...
    clean = eb.fit(pa_df, eval_season=2024)["R"]

    poison = pa_df[pa_df["season"] == 2023].copy()
    poison["season"] = 2024
    poison["woba_points"] = 9.99
    poisoned = eb.fit(pd.concat([pa_df, poison], ignore_index=True), eval_season=2024)["R"]
...
def test_g7_window_excludes_seasons_before_the_trailing_span():
    ...
    with_old = eb.fit(pd.concat([pa_df, old], ignore_index=True), eval_season=2024)["R"]
    assert np.allclose(eb.fit(pa_df, eval_season=2024)["R"]["mu"], with_old["mu"])
...
def test_g8_fit_and_predict_are_deterministic():
    ...
    first = eb.predict(full, 2024)
    second = eb.predict(full, 2024)
...
def test_g9a_covariance_is_unbiased_at_every_noise_level():
    ...
    for scale in (0.5, 2.0, 8.0):
        ...
        estimates = [
            eb.fit(synthetic_pa_table(600, REAL_MU, REAL_TAU2, 0.80, sigma2, REAL_PA,
...
def test_g9b_split_route_is_destroyed_by_a_small_sigma2_error_and_ours_is_not():
    ...
    window = window.merge(eb.batter_types(window), on="batter", how="left")
    ...
    fitted = eb.fit(pa_df, eval_season=2024, rho_min_pa=1)["R"]
    _, derived_split = eb.implied_split_constant(fitted)
...
def test_g10_switch_hitters_are_their_own_type():
    ...
    typed = eb.batter_types(pd.DataFrame(rows)).set_index("batter")["batter_type"]
...
def test_g11_rejects_wrong_shapes():
    mu, tau2, sigma2 = np.array(REAL_MU), np.array(REAL_TAU2), np.array(REAL_SIGMA2)
    matrix = eb.sigma_matrix(tau2, 0.5)
    with pytest.raises(AssertionError, match=r"\(N,2\)"):
        eb.posterior_mean(np.array([0.3, 0.3]), np.array([10.0, 10.0]), mu, matrix, sigma2)
    with pytest.raises(AssertionError, match=r"\(N,2\)"):
        eb.posterior_mean(np.array([[0.3, 0.3]]), np.array([[10.0, 10.0, 5.0]]), mu, matrix, sigma2)
...
def test_g11_predict_emits_the_claim1_contract():
    ...
    eval_rows = pa_df[pa_df["season"] == 2023].copy()
    eval_rows["season"] = 2024
    predictions = eb.predict(pd.concat([pa_df, eval_rows], ignore_index=True), 2024)

    assert list(predictions.columns) == eb.KEY + ["pred_woba"]
    assert (predictions["season"] == 2024).all()
    assert not predictions.duplicated(eb.KEY).any()
