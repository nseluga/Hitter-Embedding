...
from src.analysis import model_evaluation_probe_coverage as coverage
...
def test_covered_widens_with_target_noise():
    ...
    assert not coverage.covered(frame, 1.0, widen=False)[0]
    assert coverage.covered(frame, 1.0, widen=True)[0]
...
def test_covered_is_the_symmetric_z_interval():
    ...
    assert list(coverage.covered(frame, 2.0, widen=False)) == [True, True, False, False]
...
def test_coverage_rows_arithmetic_and_shape():
    ...
    rows = coverage.coverage_rows(frame, "unit")
    assert len(rows) == 2 * len(coverage.Z_BY_LEVEL)
    for row in rows:
...
def test_wilson_interval_stays_in_the_unit_interval():
    ...
    """
    low, high = coverage.wilson_interval(20, 20)
    assert 0.0 < low < 1.0 and high == 1.0
    low, high = coverage.wilson_interval(0, 20)
