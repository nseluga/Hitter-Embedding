...
- **Did:** Worked the seven-step plan built from `docs/phase-d5-review.md`'s D5-R14..R18 clusters: a scored per-plate-appearance fidelity check read before and after the rebuild, the cleaning pass, xwOBA in three roles, the four level contributors, the bin contest plus a 40-run retrain, claim-1 for all eight arms, gradient test (b), and the reporting fixes. Seven decision-log entries, results in `results/phase_d/`.
- **Why:** No fix in the plan could flip a gate, so the work is justified by making the null attributable rather than by rescuing the claim. Two items were load-bearing: the only sanctioned validation channel had never been able to detect a level error of the observed size, and frozen rule #2 was unsatisfied because zero of the eight arms carried a claim-1 number.
