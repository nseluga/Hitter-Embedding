...
## 2026-08-30 (Phase M) — the measurement ceiling, built end to end

**Did.** Built Phase M from `docs/phase-m-spec.md`: two new modules (`src/analysis/m_ceiling.py` for the statistics and simulation, `src/analysis/m_report.py` for the driver), 28 tests, nine artifacts in `results/phase_m/`, and a rebuilt `notebooks/08_measurement.ipynb` that reads them and recomputes nothing. Ran the §9 self-checks before reading any number: planted-τ² recovery on the reliability→ceiling map, a Monte-Carlo check of the map itself, and three reproduction gates against committed work — the unrestricted C.2 refit against Route B, E.15's pooled table, and E.5's within-stand rank correlation. All green; full repo 441 tests green. Everything is post-selection descriptive on 2024, labeled as such in every artifact; 2025 was not read.

**Why.** Phase E left an open question with a number attached to it and no verified provenance: the model reaches some fraction of what is measurable, but the ceiling depended on which τ² route you believed, the two routes disagreed by 5×, and the stabilization thresholds carried into the handoff as "~430 / >2000" had never been computed. Phase M's job was to settle the route question by pre-registration rather than by picking the flattering answer, and to confirm or correct the inherited figures.

**Found.** The headline is a bracket, not a point. Under the pre-registered primary route B′ — the C.2 fit restricted to the hitters who actually reached the 2024 eval population — τ² is 0.00039297, the within-stand ceiling is 0.2965, and the model's achieved 0.1463 is 49.3% of it. Under Route A, the 2024 empirical subtraction, the ceiling is 0.1671 and the same achieved correlation is 87.5% of it. Route B, the committed unrestricted fit, gives 0.3556 and 41.1%. Two findings explain why the bracket has to stay open. Restricting B to B′ drops τ² by a third and closes 42% of the A-to-B gap, so selection is part of the disagreement but not all of it. And Route A's pre-registered ±3% fragility band does not just widen it — at ×1.03 τ² goes negative and Route A returns no ceiling at all, meaning a 3% error in the noise model in the unfavourable direction leaves the flattering route saying there is no measurable true differential to reach.

The stabilization thresholds confirm the handoff exactly: 433 PA vs LHP under Route B (quoted "~430"), 2,184 under Route A (quoted ">2000"), 651 under B′ — against a 2024 median of 86 PA vs LHP. Charging both sides roughly doubles all three, so both conventions are now reported. On the differential itself the model has no edge: 0.1463 against C.2's 0.1458 on 545 identical rows, with the model better on LHB (0.226 vs 0.192) and worse on RHB (0.039 vs 0.085). Per stratum, the ceiling rises with exposure while the fraction reached falls — 75% at low, 46% at medium, 34% at high — the model is closest to the ceiling exactly where the ceiling is lowest. At the level (not the differential), reliability is 0.55, ceiling 0.742, achieved 0.485, fraction 65%, with a game-parity split-half cross-check landing at 0.674. Coverage reproduces E.14's 0.8555 exactly and holds between 0.856 and 0.877 across every stratum, so the miscoverage is uniform rather than concentrated where exposure is thin.

**Learned.** Three things worth carrying. First, `sqrt(reliability)` is not conservative for a rank correlation at these exposures — the direction reverses. Under a homoscedastic simulation the rank ceiling sits 3.6% below the analytic value as textbooks predict; under the real 2024 exposure profile, with a 36× spread in per-hitter sampling variance, it comes in 3.9% *above*. Rank correlation's robustness to the heavy-tailed observations that low-exposure hitters contribute buys back more than the loss of joint normality costs. Every fraction in Phase M therefore runs a few percent relatively high, and the analytic bound cannot be quoted as a safe upper limit without saying so. Second, the alarming −0.366 split-half reliability from Phase E is an artifact and not a finding: the estimator is not buggy, the observed raw half correlation sits at the 18th percentile of the τ²=0 null and the 6th of the τ²=B′ null — an ordinary draw under both — and Spearman-Brown simply magnifies a negative input it has no interpretation for. Third, a ceiling and an achieved correlation must be built on the same residualisation. Comparing a within-stand ceiling against a raw correlation produced per-stratum fractions above 1.57; the between-stand main effect E.5 identified is large enough to swamp the quantity being measured.

**Next.** The phase boundary is `/research-review` on the Phase M artifacts and the decision log, feeding it the three surviving weaknesses honestly: B′ conditions on survival to 2024 and is not a general-population τ²; the C.3-full row is absent under a fired fallback rule, so the "can ordinary ML do better" question is answered only against C.2; and every fraction inherits the few-percent optimism from the rank-vs-Pearson gap. After that, the §8 knob arm, which the spec placed after M by design.

## 2026-08-31 (Phase M close) — where the model actually stands, and the order to the abstract

**Did.** Audited the model against the full Phase C ladder on the committed artifacts, and fixed the run-out order against the Oct 1 abstract date.

**Why.** Phase M reports a fraction of a ceiling. That is not the same question as whether the model has an edge over the incumbent, and the abstract needs both answered before it can be written.

**Found.** Both pre-registered Claim 1 gates are false. On the paired bootstrap the model loses on RMSE to C.3-full and to C.2 with intervals excluding zero, and ties both on rank correlation with intervals spanning zero; it beats only the C.1 rungs. Losing on error while tying on ordering lines up with E.14's 0.856 empirical coverage against a nominal 0.95 — two independent signs that the defect is in scale, not in ordering. The 2025 refit and final claim-1 table that plan §3.1 assigns to notebook 08 have not been run, so every committed number remains post-selection descriptive on 2024.

**Learned.** The transferable contribution is the measurement apparatus rather than the model: raw correlation confounds model quality with target reliability, and the same 0.146 reads as 41% or 88% depending on a route choice that has to be fixed before the answer is visible. The project's own bracket is the worked example, including the part where the flattering route breaks under a 3% perturbation.

**Next.** Review Phase O and Phase M. Verify The Book p.157, which is decision-bearing and blocks the write-up. Build notebook 07, which must close before 08 touches 2025. Then the 2025 refit and the final claim-1 table.

---

## 2026-08-31 — Phase O and Phase M review, and the run order to the abstract

- **Did:** Ran `/research-review` on both phases against the decision log, with a §3c citation audit and two original computations — a paired hitter-level bootstrap on the M.1 differential and an M.2 stratum table rescored with C.2 as an opponent. Verdict **7/10**. Four decisions above follow from it; scratch scripts move into `src/analysis/` when the replacement table is built.
- **Why:** The 2025 refit reads a sealed season once, so a defect found after it cannot be corrected without spending the season twice. Phase O needed an audit of the 2026-08-25 fixes rather than a fresh review, and Phase M had never been reviewed.
- **Found:** Three weaknesses block the refit. No achieved correlation carries an interval, and the headline 49.3% has a 95% interval of [17%, 84%] with τ² held fixed, so the A-to-B′ bracket is narrower than the sampling error inside its own numerator. M.2 has no opponent column, and adding C.2 reverses the reading in the low stratum (78% against the model's 75%) and the high (57% against 34%), with no stratum's paired difference excluding zero. C.3-full is absent under a fallback rule whose own artifact records the refit as a supported path. Phase O's `incumbent_stands` is robust — the best challenger sits at +0.816 SE against a 2 SE bar and every identified defect was anti-conservative around a null. The citation audit passes Franks (2016) and Ma & Yarats (2021) on exact quotes, returns Brown (2008) as a partial, and leaves The Book p.157 unreachable.
- **Learned:** Warmup length is set by the second-moment memory length 1/(1−β₂), not by an epoch — 719 steps reaches full learning rate at 0.7 of that memory, before the AdamW denominator has converged. A ceiling and its numerator must share a correlation definition as well as a residualisation: the analytic bound is exact for Pearson, and the Spearman numerator needs the simulated Spearman ceiling.
- **Next:** Start the warmup arm and the C.3-full refit, then build the replacement differential table carrying RMSE, rank, both opponents, and intervals. Then the citation close-out, notebook 07, Phase V, the 2025 refit, and the abstract.
