...
- **Decision:** C.3 is an XGBoost model at the unit the claim-1 metric scores — one row per (batter, target season, pitcher hand) — whose features are the hitter's own prior-window rates sliced by context (pitcher hand as a feature, pitch group, two-strike count). Two feature sets are fitted and both reported: `outcome` (trailing side-specific wOBA and PA, own and opposite side, batter hand, service) and `full` (`outcome` plus swing/whiff/EV/LA/spray and their context slices). Training rows are target seasons 2016-2023 with the same 3-season feature window C.1 and C.2 use; the boosting-round count is chosen by early stopping on target season 2023 and the model is then refit on all eight seasons at that count. Hyperparameters are pre-registered (depth 3, heavy regularization), loss is PA-weighted squared error, labels are observed wOBA. `src/analysis/c3_gbm.py`, 18 gates.
- **Alternatives:** The 48-dim per-pitch context vector as the feature set (rejected: it carries no hitter identity by design, so it cannot emit the hitter-level quantity claim 1 grades — B.2's own finding). Early stopping on the 2024 eval frame (rejected: selection leakage into the frame every Phase C number is reported on). A hyperparameter search (rejected: same leak, and 10,548 rows is a small-data GBM). Training on C.2's posterior instead of observed wOBA (rejected: makes C.2 the teacher and the head-to-head circular). A single feature set (rejected: `outcome` alone is a strawman given B.1's process-stabilization result, and `full` alone cannot say where its skill came from).
- **Rationale:** The plan names "XGBoost with context-interaction features" without specifying the row unit; the metric's unit settles it. Pitcher hand enters as a feature rather than as two models so hand x skill interactions are learned rather than assumed. The `outcome` -> `full` pair is the first hitter-level test of the Phase D premise — B.1 established that process *stabilizes* faster than outcome, not that it *projects* better — and it cost one extra fit. Refitting on all eight seasons reuses only the round count, stated rather than hidden (the same declared compromise as B.2's shared VAL).
- **Revisit if:** Phase D needs a tuned-GBM upper bound rather than a pre-registered one, which would require its own inner split.

---

## 2026-07-28 — C.2's prior mean stays exchangeable; the confound is reported, not corrected

- **Decision:** C.2 is left exactly as logged 2026-07-27 — a single prior mean mu per (batter type, pitcher hand). No exposure-conditional mu(n) is built. The resulting confound in the C.2/C.3 comparison is documented and quantified instead.
- **Alternatives:** Adding a C.2b baseline with mu(n) fit on training seasons (rejected as scope: Phase C's job is to produce the incumbent bar, not to iterate on it). Modifying C.2 in place (rejected: reopens a logged decision and destroys the faithful-to-The-Book baseline frozen rule #1 names).
- **Rationale:** Prior exposure correlates with talent at Spearman +0.24/+0.26 (measured, `c2_exposure_talent_correlation.csv`), so C.2's exchangeability assumption is known-false in a direction that costs it: it over-predicts low-exposure hitters by +0.0169 where C.3, which has exposure as a feature, over-predicts by +0.0064. An oracle recentering of both models — removing each one's per-stratum level error using held-out actuals, therefore not a model and never scored as one (`oracle_debias` in `c_report.py`, `c3_vs_c2_oracle_debiased.csv`) — collapses C.3-full's low-exposure margin from -0.00265 (CI excludes zero, 99.2%) to -0.00050 (CI crosses zero, 71.7%). Roughly 81% of C.3's reported low-exposure advantage is a level correction C.2 is structurally forbidden from making, not projection skill C.3 has and C.2 lacks. Phase C therefore reports: C.3 is better calibrated in low exposure, and is NOT demonstrably better at ordering hitters, where C.2's rank correlation is higher (0.169 vs 0.102).
- **Revisit if:** Phase D's margin over C.3 turns out to be the same level effect, in which case an exposure-conditional prior mean must be built for both the C.2 baseline and the Phase D comparison before any claim is made.
