...
## 2026-08-30 — Route B reproduces exactly; the spec's fitting-window prose does not describe the code
- **Decision:** §10.4 does not fire. A live `c2_bivariate_eb.fit` reproduces every committed `tau2_split_derived` to twelve decimal places (L 0.0007322758887773689, R 0.0004917731167240981, S 0.000588220547130464), and the per-hitter weight-average over the M.6 population lands on the spec's pooled 0.00059034. The spec's description of Route B as a "nine-season, 2016–2024 fit" is wrong prose: `c1_trailing.TRAILING_SEASONS = 3` makes the committed fit a 2021–2023 trailing window. The value is adopted as committed and the window is recorded correctly in `m0_routes.json`; the spec text is not retro-edited.
- **Alternatives:** Treat the window mismatch as a reproduction failure and fire §10.4 (rejected: rule 4 is about a committed *value* failing to reproduce, and the value reproduces exactly — the defect is in a sentence describing the code, not in the code or the number). Refit Route B on an actual nine-season window to match the prose (rejected: that would replace the committed incumbent with a new estimate after the route rule was frozen, which is exactly the substitution pre-registration forbids).
- **Rationale:** Route B exists in the rule as a provenance row, and provenance is served by reproducing what was committed, not by producing something closer to how it was described. The window matters to one downstream reading only — B and B′ must differ in *population alone* for the B→B′ delta to be a population diagnostic — and they do: B′ uses the same three-season window with the hitter set restricted.
...
- **Revisit if:** a future stage refits C.2 on a genuinely different window, at which point B and B′ would differ on two axes and the population diagnostic would no longer isolate one.

## 2026-08-30 — §10 fallback rule 1 fires: M.1 reports C.2 only
- **Decision:** C.3-full is omitted from the M.1 differential table. `results/` holds no persisted fitted C.3 artifact and `c3_gbm.predict` calls `c3_gbm.fit` inside the call, so side-specific C.3 predictions require retraining the GBM on the 341MB labeled pitch table — the condition rule 1 names. The omission is flagged at the top level of `m0_routes.json` and `m_summary.json` with the reason attached, and no differential head is improvised.
- **Alternatives:** Retrain C.3 to fill the row (rejected: the spec pre-decided that this is out of the window's scope, and a rule fires on its stated condition, not on whether the work is convenient). Report the row as unavailable without a reason (rejected: an absent control reads as a control that lost).
- **Rationale:** Recorded with a caveat that matters for how the gap is read: the refit is a *supported existing code path* (`src/analysis/c_report.py:364`) needing no new feature code, so this is a scope boundary the spec pre-set and not a capability limit of C.3. The live question the row would have answered — whether ordinary ML reaches more of the ceiling on this cut — stays open, and C.2's row shows the model does not beat the incumbent on it either way.
...
- **Revisit if:** C.3-full is refit for any other reason, at which point the row is a scoring pass and can be added without re-opening the route rule.

## 2026-08-30 — sqrt(reliability) is not a conservative bound on the rank ceiling at these exposures
- **Decision:** Every fraction-of-ceiling figure in Phase M is labeled as running a few percent relatively high, and the Monte-Carlo rank ceiling is emitted beside the analytic one in `m0_routes.json` rather than the analytic value standing alone. The analytic map is retained as the reported ceiling; it is not replaced by the simulated one.
- **Alternatives:** Report the simulated rank ceiling as the headline (rejected: it is a 300-draw estimate with sd 0.042 at the primary route, so substituting it would trade a small known bias for a larger sampling error, and it would break comparability with E.15's committed table). Assert the textbook direction and move on (rejected: the §9.1 check measured the opposite and the assertion would have been false).
- **Rationale:** `sqrt(reliability)` is derived under joint normality and bounds the Pearson correlation; the project scores with a rank correlation. Under a homoscedastic simulation the rank ceiling sits 3.6% below the analytic value as the textbook predicts. Under the real 2024 exposure profile — a ~36× spread in per-hitter sampling variance — it comes in 3.9% *above* it: rank correlation is robust to the heavy-tailed observations low-exposure hitters contribute, and that robustness buys back more than joint normality costs. The Pearson arm of the same simulation matches the analytic value, so the reliability→ceiling map itself is verified exactly and only the rank-vs-Pearson step is approximate.
...
- **Revisit if:** a ceiling is ever quoted on a population with a materially flatter exposure profile, where the sign of the gap reverts to the textbook direction.

## 2026-08-30 — Route C is retired: the negative reliability is an artifact, not a finding
- **Decision:** Route C's verdict is `uninformative_inside_both_nulls`, and it is reported as evidence in neither direction. §10.2 does not fire — the observed value is not outside both nulls, so the noise model is not impugned and Route A keeps its status as the reported sensitivity.
- **Alternatives:** Read the LHB value as support for small τ² and hence for Route A (rejected: it is inside the τ²=B′ null as well, so it separates nothing). Report it unexplained (rejected: the simulation explains it, and leaving a −0.366 in the record unexplained invites a reviewer to draw the conclusion the data does not support).
- **Rationale:** The audit found no bug in `split_half_reliability` — the split is on game parity so the halves are disjoint games, the pairing is per hitter, the exposure floor applies to both halves, and the pooled row is centred by stand. The null simulation, built on each hitter's real half-length exposures and E.15's noise model, places the observed raw LHB half correlation of −0.155 at the 18th percentile of the τ²=0 null and the 6th of the τ²=B′ null: an ordinary draw under both. The alarming −0.366 is Spearman-Brown applied to a negative input, where 2r/(1+r) has no reliability interpretation and simply magnifies the magnitude. At a median 43 PA vs LHP per half the estimator has too much sampling variance to discriminate anything, which is the bounded outcome the spec allowed for.
...
- **Revisit if:** a split-half estimate is ever wanted as more than a diagnostic, which would require per-half exposures several times larger than 2024 provides.

## 2026-08-30 — The headline is a bracket: selection closes 42% of the A-to-B gap, and Route A breaks under its own band
- **Decision:** The Phase M headline reports the ceiling and the achieved fraction under B′ and A side by side, B′ first, and is stated as a bracket rather than a point. Under B′ the within-stand ceiling is 0.2965 and the model reaches 49.3%; under A it is 0.1671 and the model reaches 87.5%. Route B (0.3556, 41.1%) is the provenance row.
- **Alternatives:** Quote B′ alone now that it is non-degenerate (rejected: the route rule requires A beside it, and A's band is a material part of what the reader needs). Quote A's 88% (rejected: it is the flattering end of a bracket whose other end is 49%, and route A is the fragile end).
- **Rationale:** Two things the build settled make the bracket the only honest form. First, the B→B′ diagnostic: restricting the C.2 fit to hitters who reached the 2024 eval population drops τ² by 33% and closes 41.7% of the B-to-A gap, so *part* of the A-vs-B disagreement is population and the remainder is window or estimator — neither route explains the other away. Second, Route A's ±3% fragility band does not merely widen the estimate, it breaks it: at ×0.97 the ceiling rises to 0.2389, and at ×1.03 τ² goes negative and Route A returns no ceiling at all. B′ conditions on survival to 2024 and is labeled as such; it is the right population for a claim about 2024 eval hitters and is not a general-population τ².
...
- **Revisit if:** a τ² estimate becomes available on a population that does not condition on survival to the eval season, which would let the bracket close from the B′ end.

## 2026-08-30 — Per-stratum fractions compare within-stand quantities, and the M.2 precision clause is conditional
- **Decision:** M.2's achieved correlation is the **within-stand** rank correlation, computed through E.15's own residualisation, and the fraction of ceiling is formed from it. The raw correlation is emitted alongside, labeled as not comparable to a within-stand ceiling. Separately, the precision clause is applied as written — the narrowest-relative-CI tie-break fires **only if** the default stratum's own 95% CI includes zero. No stratum's CI includes zero on the real table, so the illustration stays on `low`.
- **Alternatives:** Compare against the raw per-stratum correlation (rejected: τ² is a within-stand variance, so the raw figure — which E.5 showed is mostly the between-stand main effect — produces fractions above 1, e.g. 1.57 in the low stratum; that is the apples-to-oranges error E.5's decomposition exists to prevent). Take the narrowest relative CI unconditionally (rejected: that reads the clause as a standing preference for precision, which would move the illustration off the stratum the thesis is graded on for a 1.6-point difference in relative CI width — the clause exists to handle an unusable interval, not to rank usable ones).
- **Rationale:** τ² is not refit per stratum — a per-stratum refit is a new estimator and was not authorised — so the common B′ fit is applied against each stratum's own sampling-variance profile, as E.15 applies a common fit across stands. On that construction the ceiling rises monotonically with exposure (low 0.2531, medium 0.3113, high 0.3248) while the fraction reached falls (75%, 46%, 34%): the model is closest to the ceiling exactly where the ceiling is lowest. Route A per stratum is degenerate in `medium` outright and in 7.5–20% of bootstrap draws elsewhere, so its intervals are reported as conditional on non-degeneracy with the degenerate share beside them, and no draw is clipped to zero.
...
- **Revisit if:** a bootstrap on a re-run puts the low stratum's CI across zero, at which point the clause fires and the illustration moves to the narrowest-relative-CI stratum with the move stated in the artifact.

## 2026-08-30 — The stabilization threshold is reported under both conventions
- **Decision:** PA* is emitted twice: `pa_star_weak_side`, charging only PAs faced vs LHP, and `pa_star_both_sides`, charging both sides growing together. The weak-side convention confirms the handoff's unverified figures — 433 PA under Route B (quoted as "~430") and 2,184 under Route A (quoted as ">2000") — and B′ lands at 651. The both-sides figures are roughly double throughout (877 / 4,419 / 1,317).
- **Alternatives:** Report the weak-side number alone (rejected: it is the convention `implied_split_constant` uses and the one the handoff quoted, but a reader comparing PA* against a hitter's actual season will assume exposure accumulates on both sides, and would read the threshold as half what it is). Report the both-sides number alone (rejected: it silently breaks continuity with `n_star_split_implied` in the committed C.2 artifacts).
