...
- **Alternatives:** Promote `lr1e3_warm` on its +0.8 SE and confirm at five seeds (rejected: the promotion rule was fixed in code before any run existed, and its two seeds were 1.02567 and 1.02585 — a 1.27e-4 spread, wider than the margin itself). Extend the grid to a wider learning-rate range or a longer warmup (rejected: the four off-incumbent arms fail monotonically and by wide margins, so the grid brackets the optimum rather than missing it).
- **Rationale:** `incumbent_stands` is a real Phase O result and the one the spec named as such — the answer to "the learning rate was fixed at 1e-3 across all 119 runs and never varied" is now that it was varied over a 10× range and 1e-3 was already best. This also settles the O.2 diagnostic without new computation: the pre-registered prediction was that the rarest quintile's embedding norm would fall toward the most-exposed quintile's on the tuned build, the tuned build *is* the build `gradient_b` was measured on, and the exposure–norm gradient is therefore intact by construction. That is a null, and it is reported as one.
...
- **Revisit if:** Phase M's schedule leaves room for the quarantined-knob arm — the 2026-08-20 quarantine entry's revisit condition is now met, since an intact gradient (b) means learning rate and warmup were not the lever, and that arm's strength must be fixed before any claim-1 number is read and checked by re-measuring F.4's identity share.
